...
DecoupledBPUWithFTB::DecoupledBPUWithFTB(const DecoupledBPUWithFTBParams &p)
    : BPredUnit(p), enableLoopBuffer(p.enableLoopBuffer),
      enableLoopPredictor(p.enableLoopPredictor),
      enableJumpAheadPredictor(p.enableJumpAheadPredictor),
      fetchTargetQueue(p.ftq_size), fetchStreamQueueSize(p.fsq_size),
      numBr(p.numBr), historyBits(p.maxHistLen), uftb(p.uftb), ftb(p.ftb),
      tage(p.tage), ittage(p.ittage), ras(p.ras), uras(p.uras),
      enableDB(p.enableBPDB), numStages(p.numStages), historyManager(p.numBr),
      dbpFtbStats(this, p.numStages, p.fsq_size), preBranchType(ALL), enabletbit(p.enabletbit),
      enablenbt(p.enableNBT), enablenst(p.enableNST) {
    if (enabletbit) {
        tbit = new TBIT();
    }
    if (enableDB) {
    ...
    for (unsigned i = 0; i < numStages; i++) {
        ...
        predsOfEachStage[i].condIsHigh.resize(numBr, false);
        predsOfEachStage[i].ftbTaken.resize(numBr, false);
    }
    ...
    registerExitCallback([this]() {
        ...
        *out_handle->stream() << "startPC"
        ...
        std::sort(topMisPredPC.begin(), topMisPredPC.end(),
                  [](const std::pair<std::pair<Addr, Addr>, int> &a,
                     const std::pair<std::pair<Addr, Addr>, int> &b) {
                      return a.second > b.second;
                  });
        for (auto &it : topMisPredPC) {
            *out_handle->stream()
                << std::hex << it.first.first << " " << it.first.second << " "
                << std::dec << it.second << std::endl;
        }
        simout.close(out_handle);

        // at a per branch basis
        out_handle = simout.create("topMispredictsByBranch.csv", false, true);
        std::vector<std::tuple<Addr, int, int, int, double, int, int, int>>
            topMisPredPCByBranch;
        *out_handle->stream() << "pc"
                              << ","
                              << "type"
                              << ","
                              << "mispredicts"
        ...
        for (auto &it : topMispredictsByBranch) {
            topMisPredPCByBranch.push_back(
                std::make_tuple(it.first.first, it.first.second,
                                it.second.first.first, it.second.second,
                                (double)(it.second.first.first * 1000) /
                                    (double)it.second.second,
                                it.second.first.second.at(DIR_WRONG),
                                it.second.first.second.at(TARGET_WRONG),
                                it.second.first.second.at(NO_PRED)));
        }
        std::sort(
            topMisPredPCByBranch.begin(), topMisPredPCByBranch.end(),
            [](const std::tuple<Addr, int, int, int, double, int, int, int> &a,
               const std::tuple<Addr, int, int, int, double, int, int, int>
                   &b) { return std::get<2>(a) > std::get<2>(b); });
        for (auto &it : topMisPredPCByBranch) {
            *out_handle->stream() << std::dec << std::get<0>(it) << std::dec
                                  << "," << std::get<1>(it) << ","
                                  << std::get<2>(it) << "," << std::get<3>(it);
            *out_handle->stream()
                << "," << (int)std::get<4>(it) << "," << (int)std::get<5>(it)
                << "," << (int)std::get<6>(it) << "," << (int)std::get<7>(it)
                << std::endl;
        }
        simout.close(out_handle);
        ...
        *out_handle->stream() << "pc"
        ...
        std::sort(
            ...
            [](const std::tuple<Addr, int, int, int, double, int, int, int> &a,
               const std::tuple<Addr, int, int, int, double, int, int, int>
                   &b) { return std::get<4>(a) > std::get<4>(b); });
        ...
        for (auto &it : topMisPredPCByBranch) {
            ...
            *out_handle->stream() << std::hex << std::get<0>(it) << std::dec
                                  << " " << std::get<1>(it) << " "
                                  << std::get<2>(it) << " " << std::get<3>(it);
            *out_handle->stream()
                << " " << (int)std::get<4>(it) << " " << (int)std::get<5>(it)
        ...
        *out_handle->stream() << std::endl;
        for (auto &it : topMispredictsByBranchByPhase) {
            ...
            int numEverTakenStaticBranches =
                takenBranchesByPhase[phaseID].size();
            ...
            *out_handle->stream()
                << phaseID << " " << numStaticBranches << " "
                << numEverTakenStaticBranches << " " << totalMispredicts;
            // sort by mispredicts
            std::sort(temp.begin(), temp.end(),
                      [&](const MispredRecord &a, const MispredRecord &b) {
                          return gem5::branch_prediction::ftb_pred::
                                     DecoupledBPUWithFTB::getMispredCount(a) >
                                 gem5::branch_prediction::ftb_pred::
                                     DecoupledBPUWithFTB::getMispredCount(b);
                      });
            for (int i = 0; i < outputTopN; i++) {
                *out_handle->stream()
                    << " " << std::hex << temp[i].first.first; // pc
                *out_handle->stream()
                    << " " << std::dec << temp[i].first.second; // type
                *out_handle->stream()
                    << " " << std::dec
                    << getMispredCount(temp[i]); // mispred count
        ...
        *out_handle->stream() << std::endl;
        for (auto &it : topMispredictsByBranchBySubPhase) {
            int numStaticBranches = it.size();
            int numEverTakenStaticBranches =
                takenBranchesBySubPhase[phaseID].size();
            int totalMispredicts = 0;
            std::vector<MispredRecord> temp;
            for (auto &it2 : it) {
                temp.push_back(it2);
        ...
        //                       << " not use loop: " << notUseLoop <<
        //                       std::endl;
        *out_handle->stream() << "Hist"
                              << " "
                              << "count" << std::endl;
        ...
        for (const auto &entry : topMisPredHistVec) {
            *out_handle->stream() << std::hex << entry.first << " " << std::dec
        ...
        //             << "--->"
        //             << (misPred ? entry.second.exeTarget :
        //             entry.second.predTarget)
        //             << std::dec
        ...
        for (auto it : tempVec) {
            *out_handle->stream() << std::oct << it.second << " " << std::hex
                                  << it.first << std::endl;
        }
        ...
        out_handle = simout.create("fsqEntryCommittedInstNumDistsByPhase.txt",
                                   false, true);
        *out_handle->stream() << "phaseID";
        for (int i = 0; i <= 16; i++) {
            *out_handle->stream() << " " << i;
        ...
        *out_handle->stream() << " "
        ...
        for (auto &it : fsqEntryNumCommittedInstDistByPhase) {
            ...
            *out_handle->stream()
                << " " << (double)phaseSizeByInst / (double)numFsqEntries
                << std::endl;
            phaseID++;
        }
        simout.close(out_handle);
        ...
        out_handle = simout.create("fsqEntryFetchedInstNumDistsByPhase.txt",
                                   false, true);
        *out_handle->stream() << "phaseID";
        for (int i = 0; i <= 16; i++) {
            *out_handle->stream() << " " << i;
        ...
        *out_handle->stream() << std::endl;
        ...
        for (auto &it : FTBEntriesByPhase) {
            ...
            *out_handle->stream() << " " << it.size();
            ...
            for (auto &rec : it) {
                ftbEntryTempVec.push_back(std::make_tuple(
        ...
        // out_handle = simout.create("entryTargetSlots.csv", false, true);
        // for (auto& it: slotTargetMap) {
        //     for (auto& it2: it.second) {
        //         *out_handle->stream() << it.first << "," << it2.first << ","
        //         << it2.second << std::endl;
        //     }
        // }
        // simout.close(out_handle);
...
DecoupledBPUWithFTB::DBPFTBStats::DBPFTBStats(statistics::Group *parent,
    ...
    : statistics::Group(parent),
      ...
      ADD_STAT(staticBranchNum, statistics::units::Count::get(),
               "the number of all (different) static branches"),
      ADD_STAT(
          staticBranchNumEverTaken, statistics::units::Count::get(),
      ...
      ADD_STAT(commitPredsFromEachStage, statistics::units::Count::get(),
               "the number of preds of each stage that account for a committed "
               "stream"),
      ADD_STAT(fsqEntryDist, statistics::units::Count::get(),
               "the distribution of number of entries in fsq"),
      ADD_STAT(fsqEntryEnqueued, statistics::units::Count::get(),
               "the number of fsq entries enqueued"),
      ADD_STAT(fsqEntryCommitted, statistics::units::Count::get(),
               "the number of fsq entries committed at last"),
      ...
      ADD_STAT(ftbEntriesWithDifferentStart, statistics::units::Count::get(),
               "number of ftb entries with different start PC"),
      ADD_STAT(
          ftbEntriesWithOnlyOneJump, statistics::units::Count::get(),
          "number of ftb entries with different start PC starting with a jump"),
      ...
      ADD_STAT(predLoopPredictorExit, statistics::units::Count::get(),
               "loop predictor exits at pred"),
      ADD_STAT(predLoopPredictorUnconfNotExit, statistics::units::Count::get(),
               "loop predictor does not exit at pred because of unconf"),
      ADD_STAT(predLoopPredictorConfFixNotExit, statistics::units::Count::get(),
               "loop predictor confident and fix other predictor not taken in "
               "non-exit loop branch at pred"),
      ADD_STAT(
          predFTBUnseenLoopBranchInLp, statistics::units::Count::get(),
          "loop predictor recorded loop branch that is not in ftb encountered"),
      ADD_STAT(predFTBUnseenLoopBranchExitInLp, statistics::units::Count::get(),
               "loop predictor recorded loop branch that is not in ftb "
               "encountered and exited"),
      ADD_STAT(commitLoopPredictorExit, statistics::units::Count::get(),
               "loop predictor pred loop exits detected at commit"),
      ADD_STAT(commitLoopPredictorExitCorrect, statistics::units::Count::get(),
               "loop predictor correctly pred loop exits detected at commit"),
      ADD_STAT(commitLoopPredictorExitWrong, statistics::units::Count::get(),
               "loop predictor wrongly pred loop exits detected at commit"),
      ADD_STAT(commitFTBUnseenLoopBranchInLp, statistics::units::Count::get(),
               "loop predictor recorded loop branch that is not in ftb "
               "encountered at commit"),
      ADD_STAT(commitFTBUnseenLoopBranchExitInLp,
               statistics::units::Count::get(),
               "loop predictor recorded loop branch that is not in ftb "
               "encountered and exited at commit"),
      ADD_STAT(commitLoopPredictorConfFixNotExit,
               statistics::units::Count::get(),
               "loop predictor confident and fix other predictor not taken in "
               "non-exit loop branch at commit"),
      ADD_STAT(commitLoopPredictorConfFixNotExitCorrect,
               statistics::units::Count::get(),
               "loop predictor confident and fix other predictor not taken in "
               "non-exit loop branch correctly at commit"),
      ADD_STAT(commitLoopPredictorConfFixNotExitWrong,
               statistics::units::Count::get(),
               "loop predictor confident and fix other predictor not taken in "
               "non-exit loop branch wrongly at commit"),
      ADD_STAT(
          commitLoopExitLoopPredictorNotPredicted,
          statistics::units::Count::get(),
          "loop exit detected at commit that loop predictor did not pred exit"),
      ADD_STAT(commitLoopExitLoopPredictorNotConf,
               statistics::units::Count::get(),
               "loop exit detected at commit that loop predictor did not pred "
               "exit because of unconfident"),
      ADD_STAT(controlSquashOnLoopPredictorPredExit,
               statistics::units::Count::get(),
               "cotrol squash on loop predictor pred loop exits"),
      ADD_STAT(nonControlSquashOnLoopPredictorPredExit,
               statistics::units::Count::get(),
               "non-cotrol squash on loop predictor pred loop exits"),
      ADD_STAT(trapSquashOnLoopPredictorPredExit,
               statistics::units::Count::get(),
               "trap squash on loop predictor pred loop exits"),
      ADD_STAT(predBlockInLoopBuffer, statistics::units::Count::get(),
               "predicted block is from loop buffer"),
      ADD_STAT(predDoubleBlockInLoopBuffer, statistics::units::Count::get(),
               "predicted double block is from loop buffer"),
      ADD_STAT(squashOnLoopBufferPredBlock, statistics::units::Count::get(),
               "squash on loop buffer provided block"),
      ADD_STAT(squashOnLoopBufferDoublePredBlock,
               statistics::units::Count::get(),
               "squash on loop buffer provided double block"),
      ADD_STAT(commitBlockInLoopBuffer, statistics::units::Count::get(),
               "committed block is from loop buffer"),
      ADD_STAT(commitDoubleBlockInLoopBuffer, statistics::units::Count::get(),
               "committed double block is from loop buffer"),
      ADD_STAT(commitBlockInLoopBufferSquashed, statistics::units::Count::get(),
               "committed block is from loop buffer but squashed"),
      ADD_STAT(commitDoubleBlockInLoopBufferSquashed,
               statistics::units::Count::get(),
               "committed double block is from loop buffer but squashed"),
      ADD_STAT(commitLoopBufferEntryInstNum, statistics::units::Count::get(),
               "commit block from loop buffer, buffer entry has inst num"),
      ADD_STAT(
          commitLoopBufferDoubleEntryInstNum, statistics::units::Count::get(),
          "commit double block from loop buffer, buffer entry has inst num"),
      ADD_STAT(predJATotalSkippedBlocks, statistics::units::Count::get(),
               "jump ahead skipped total block numbers at pred"),
      ADD_STAT(commitJATotalSkippedBlocks, statistics::units::Count::get(),
               "jump ahead skipped total block numbers at commit"),
      ADD_STAT(squashOnJaHitBlocks, statistics::units::Count::get(),
               "total number of squashes on ja hit blocks"),
      ADD_STAT(controlSquashOnJaHitBlocks, statistics::units::Count::get(),
               "total number of control squashes on ja hit blocks"),
      ADD_STAT(nonControlSquashOnJaHitBlocks, statistics::units::Count::get(),
               "total number of non-control squashes on ja hit blocks"),
      ADD_STAT(trapSquashOnJaHitBlocks, statistics::units::Count::get(),
               "total number of trap squashes on ja hit blocks"),
      ADD_STAT(commitSquashedOnJaHitBlocks, statistics::units::Count::get(),
               "total number of squashes on ja hit committed blocks"),
      ADD_STAT(commitControlSquashedOnJaHitBlocks,
               statistics::units::Count::get(),
               "total number of control squashes on ja hit committed blocks"),
      ADD_STAT(
          commitNonControlSquashedOnJaHitBlocks,
          statistics::units::Count::get(),
          "total number of non-control squashes on ja hit committed blocks"),
      ADD_STAT(commitTrapSquashedOnJaHitBlocks, statistics::units::Count::get(),
               "total number of trap squashes on ja hit committed blocks"),
      ADD_STAT(predJASkippedBlockNum, statistics::units::Count::get(),
               "distribution of ja skipped block numbers at pred"),
      ADD_STAT(commitJASkippedBlockNum, statistics::units::Count::get(),
               "distribution of ja skipped block numbers at commit"),
      ADD_STAT(predTimes, statistics::units::Count::get(),
               "total number of predict times"),
      ADD_STAT(finalPredHit, statistics::units::Count::get(),
               "total number of predictor hit times"),
      ADD_STAT(finalPredMiss, statistics::units::Count::get(),
               "total number of predictor miss times"),
      ADD_STAT(condSaveTime, statistics::units::Count::get(),
               "reduce condition predictor run times"),
      ADD_STAT(indirectSaveTime, statistics::units::Count::get(),
               "reduce indirect predictor run times"),
      ADD_STAT(noBranchStream, statistics::units::Count::get(),
               "uncontrol stream of no branch"),
      ADD_STAT(indirectStream, statistics::units::Count::get(),
               "uncontrol stream of indirect"),
      ADD_STAT(condStream, statistics::units::Count::get(),
               "uncontrol stream of cond"),
      ADD_STAT(directStream, statistics::units::Count::get(),
               "uncontrol stream of direct"),
      ADD_STAT(allStream, statistics::units::Count::get(),
               "uncontrol stream of all"),
      ADD_STAT(noBranch1, statistics::units::Count::get(),
               "uncontrol stream of all"),
      ADD_STAT(noBranch2, statistics::units::Count::get(),
               "uncontrol stream of all"),
      ADD_STAT(noBranch3, statistics::units::Count::get(),
               "uncontrol stream of all"),
      ADD_STAT(noBranch4, statistics::units::Count::get(),
               "uncontrol stream of all"),
      ADD_STAT(noBranch5, statistics::units::Count::get(),
               "uncontrol stream of all"),
      ADD_STAT(noBranchAbove, statistics::units::Count::get(),
               "uncontrol stream of all"),
      ADD_STAT(decodeAllStream, statistics::units::Count::get(),
               "decode all stream"),
      ADD_STAT(containBranchStream, statistics::units::Count::get(),
               "contain branch stream") {
...
DecoupledBPUWithFTB::BpTrace::BpTrace(FetchStream &stream,
                                      const DynInstPtr &inst, bool mispred) {
    _tick = curTick();
    ...
    set(stream.startPC, pc, info.getType(), inst->branching(), mispred,
        fallThru, stream.predSource, target);
    // for (auto it = _uint64_data.begin(); it != _uint64_data.end(); it++) {
    //     printf("%s: %ld\n", it->first.c_str(), it->second);
    // }
}

void DecoupledBPUWithFTB::tick() {
    ...
    if (!receivedPred && !streamQueueFull()) {
        if (!enableLoopBuffer || (enableLoopBuffer && !lb.isActive())) {
            ...
            dbpFtbStats.predTimes++;
            // 进行预测
            for (int i = 0; i < numComponents; i++) {
                components[i]->putPCHistory(s0PC, s0History, predsOfEachStage);
            }
            directStream->putPCHistory(s0PC, s0History, predsOfEachStage);
        } else {
            DPRINTF(LoopBuffer,
                    "Do not query bpu when loop buffer is active\n");
            DPRINTF(DecoupleBP,
                    "Do not query bpu when loop buffer is active\n");
        }
    ...
    if (enableLoopBuffer && !lb.isActive() &&
        ...
        !lb.streamBeforeLoop
             .resolved) { // do not activate loop buffer right after squash
...
void DecoupledBPUWithFTB::generateFinalPredAndCreateBubbles() {
    ...
    if (!enableLoopBuffer || (enableLoopBuffer && !lb.isActive())) {
        ...
        finalPred.ftbValid = predsOfEachStage[0].valid;
        for (int i = 0; i < numBr; i++) {
            finalPred.ftbTaken[i] = predsOfEachStage[0].condTakens[i];
        }
        receivedPred = true;
        ...
        if (taken_slot_s1.isReturn) {
            for (int i = 0; i < predsOfEachStage[1].ftbEntry.slots.size();
                 i++) {
                DPRINTF(FTBuRAS, "s2 slot valid %d isCond %d\n",
                        predsOfEachStage[1].ftbEntry.slots[i].valid,
                        predsOfEachStage[1].ftbEntry.slots[i].isCond);
            }
            ...
            for (int i = 0; i < predsOfEachStage[0].ftbEntry.slots.size();
                 i++) {
                DPRINTF(FTBuRAS, "s1 slot valid %d isCond %d\n",
                        predsOfEachStage[0].ftbEntry.slots[i].valid,
                        predsOfEachStage[0].ftbEntry.slots[i].isCond);
            }
            if (!taken_slot_s0.isReturn) {
                DPRINTF(FTBuRAS,
                        "pc %llx pred to be ret on s2, but not on s1\n",
                        predsOfEachStage[1].bbStart);
            }
            if (predsOfEachStage[1].returnTarget !=
                predsOfEachStage[0].returnTarget) {
                DPRINTF(FTBuRAS,
                        "pc %llx pred to be ret but s1 target %llx, s2 target "
                        "%llx\n",
                        predsOfEachStage[1].bbStart,
                        predsOfEachStage[0].returnTarget,
                        predsOfEachStage[1].returnTarget);
            }
        }
        ...
        receivedPred = true;
        DPRINTF(LoopBuffer,
                "Do not generate final pred when loop buffer is active\n");
        DPRINTF(DecoupleBP,
                "Do not generate final pred when loop buffer is active\n");
    }

    DPRINTF(Override,
            "Ends generateFinalPredAndCreateBubbles(), numOverrideBubbles is "
            "%d, receivedPred is set true.\n",
            numOverrideBubbles);
}

bool DecoupledBPUWithFTB::trySupplyFetchWithTarget(Addr fetch_demand_pc,
                                                   bool &fetch_target_in_loop) {
    return fetchTargetQueue.trySupplyFetchWithTarget(fetch_demand_pc,
                                                     fetch_target_in_loop);
}
...
DecoupledBPUWithFTB::decoupledPredict(const StaticInstPtr &inst,
    ...
    if (in_loop) {
        DPRINTF(LoopBuffer,
                "current loop iter %d, loop_iter %d, loop_exit %d\n",
        ...
        if (raw_taken) {
            if (current_loop_iter >= loop_iter - 1) {
                ...
                if (loop_exit) {
                    ...
                    lb.tryUnpin();
                    DPRINTF(LoopBuffer,
                            "modifying taken to false because of loop exit\n");
                }
            }
...
void DecoupledBPUWithFTB::controlSquash(
    unsigned target_id, unsigned stream_id, const PCStateBase &control_pc,
    const PCStateBase &corr_target, const StaticInstPtr &static_inst,
    unsigned control_inst_size, bool actually_taken, const InstSeqNum &seq,
    ThreadID tid, const unsigned &currentLoopIter, bool fromDecode) {
    ...
    if (squashing_stream_it == fetchStreamQueue.end()) {
        ...
        // assert(fetchStreamQueue.rbegin()->second.getNextStreamStart() ==
        // MaxAddr);
    ...
    if (!fromDecode) {
        DPRINTF(MDEBUG, "control squash: %ld, recover pc: %ld taken: %d\n",
                stream.startPC, s0PC, actually_taken);
    }
    // recover pre branch type
    preBranchType = ALL;
    straightValid = false;
    if (stream.isExit) {
    ...
    if (control_pc.instAddr() == ObservingPC ||
        control_pc.instAddr() == ObservingPC2) {
        debugFlagOn = true;
    }

    DPRINTF(DecoupleBPHist, "stream start=%#lx, predict on hist: %s\n",
            stream.startPC, stream.history);
    ...
    DPRINTF(DecoupleBP || debugFlagOn,
            ...
            target_id, stream_id, control_pc.instAddr(), corr_target.instAddr(),
            is_conditional, is_indirect, actually_taken, seq);

    dumpFsq("Before control squash");

    // streamLoopPredictor->restoreLoopTable(stream.mruLoop);
    // streamLoopPredictor->controlSquash(stream_id, stream,
    ...
    stream.exeBranchInfo =
        BranchInfo(control_pc.instAddr(), corr_target.instAddr(), static_inst,
                   control_inst_size);
    ...
    if (enableLoopPredictor) {
        ...
        // we should check if the numBr possible loop branches should be
        // recovered
        for (int i = 0; i < numBr; ++i) {
            // loop branches behind the squashed branch should be recovered
            if (stream.loopRedirectInfos[i].e.valid &&
                control_pc.instAddr() <=
                    stream.loopRedirectInfos[i].branch_pc) {
                DPRINTF(DecoupleBP, "Recover loop predictor for %#lx\n",
                        stream.loopRedirectInfos[i].branch_pc);
                lp.recover(stream.loopRedirectInfos[i], actually_taken,
                           control_pc.instAddr(), true, false, currentLoopIter);
            }
        }
        for (auto &info : stream.unseenLoopRedirectInfos) {
            if (info.e.valid && control_pc.instAddr() <= info.branch_pc) {
                DPRINTF(DecoupleBP,
                        "Recover loop predictor for unseen branch %#lx\n",
                        info.branch_pc);
                lp.recover(info, actually_taken, control_pc.instAddr(), true,
                           false, currentLoopIter);
            }
        }
    }
    ...
    DPRINTF(DecoupleBPHist, "Recover history %s\nto %s\n", s0History,
            stream.history);
    ...
    std::tie(real_shamt, real_taken) = stream.getHistInfoDuringSquash(
        control_pc.instAddr(), is_conditional, actually_taken, numBr);
    for (int i = 0; i < numComponents; ++i) {
    ...
    DPRINTF(DecoupleBPHist, "Shift in history %s\n", s0History);
...
void DecoupledBPUWithFTB::nonControlSquash(unsigned target_id,
                                           unsigned stream_id,
    ...
    if (enableLoopPredictor) {
        ...
        for (int i = 0; i < numBr; ++i) {
            // loop branches behind the squashed branch should be recovered
            if (stream.loopRedirectInfos[i].e.valid &&
                inst_pc.instAddr() <= stream.loopRedirectInfos[i].branch_pc) {
                DPRINTF(DecoupleBP, "Recover loop predictor for %#lx\n",
                        stream.loopRedirectInfos[i].branch_pc);
                lp.recover(stream.loopRedirectInfos[i], false,
                           inst_pc.instAddr(), false, false, currentLoopIter);
            }
        }
        for (auto &info : stream.unseenLoopRedirectInfos) {
            if (info.e.valid && inst_pc.instAddr() <= info.branch_pc) {
                DPRINTF(DecoupleBP,
                        "Recover loop predictor for unseen branch %#lx\n",
                        info.branch_pc);
                lp.recover(info, false, inst_pc.instAddr(), false, false,
                           currentLoopIter);
            }
        }
    }
    ...
    std::tie(real_shamt, real_taken) =
        stream.getHistInfoDuringSquash(inst_pc.instAddr(), false, false, numBr);
    for (int i = 0; i < numComponents; ++i) {
        components[i]->recoverHist(s0History, stream, real_shamt, real_taken);
    }
    histShiftIn(real_shamt, real_taken, s0History);
...
void DecoupledBPUWithFTB::trapSquash(unsigned target_id, unsigned stream_id,
                                     Addr last_committed_pc,
...
void DecoupledBPUWithFTB::update(unsigned stream_id, ThreadID tid) {
    ...
    while (it != fetchStreamQueue.end() && stream_id >= it->first) {
        ...
        if (miss_predicted) {
            DPRINTF(FTBITTAGE || (stream.squashPC == 0x1e0eb6),
                    "miss predicted stream.startAddr=%#lx\n", stream.startPC);
        }
        ...
        if (stream.isHit) {
            ...
            if (stream.exeTaken) {
                dbpFtbStats.ftbMiss++;
                DPRINTF(FTB,
                        "FTB miss detected when update, stream start %#lx, "
                        "predTick %lu, printing branch info:\n",
                        stream.startPC, stream.predTick);
                auto &slot = stream.exeBranchInfo;
                DPRINTF(FTB,
                        "    pc:%#lx, size:%d, target:%#lx, cond:%d, "
                        "indirect:%d, call:%d, return:%d\n",
                        slot.pc, slot.size, slot.target, slot.isCond,
                        slot.isIndirect, slot.isCall, slot.isReturn);
            }
            if (stream.falseHit) {
        ...
        if (stream.isHit || stream.exeTaken) {
            ...
            if (it == totalFTBEntries.end()) {
                ...
                if (ftb_entry.slots.size() == 1) {
                    if (ftb_entry.slots[0].pc == stream.startPC &&
                        ftb_entry.slots[0].isUncond()) {
        ...
        BranchType ftbBranchType;
        ftbBranchType = uftb->setBranchType(stream, ftbBranchType, false);
        ftb->setBranchType(stream, ftbBranchType, true);
        directStream->update(stream);

        if (enableJumpAheadPredictor) {
            if (stream.isHit || stream.exeTaken ||
                stream.squashType != SQUASH_NONE) {
                // this block predicted, if we already recorded enough non-pred
                // blocks, then we should write into storage update predicted
                // blocks of ja info
        ...
        DPRINTF(LoopPredictor,
                "at commit fsqid %d, real_branch_pc %#lx, squash type %d, loop "
                "predcition infos:\n",
        ...
        for (int i = 0; i < numBr; ++i) {
            ...
            DPRINTF(LoopPredictor,
                    "    branch_pc %#lx, end_loop %d, specCnt %d, tripCnt %d, "
                    "conf %d\n",
                    lp_info.branch_pc, lp_info.end_loop, lp_info.e.specCnt,
                    lp_info.e.tripCnt, lp_info.e.conf);
            if (stream.fixNotExits[i]) {
                dbpFtbStats.commitLoopPredictorConfFixNotExit++;
                if (stream.squashType == SQUASH_CTRL &&
                    stream.squashPC == lp_info.branch_pc) {
        ...
        if (stream.fromLoopBuffer) {
            ...
            if (instNum > 0) {
                ...
                if (stream.isDouble) {
                    dbpFtbStats.commitLoopBufferDoubleEntryInstNum.sample(
                        instNum, 1);
                }
            }
        }
        ...
        if (stream.squashType == SQUASH_CTRL) {
            auto find_it = topMispredicts.find(
                std::make_pair(stream.startPC, stream.exeBranchInfo.pc));
            if (find_it == topMispredicts.end()) {
                topMispredicts[std::make_pair(stream.startPC,
                                              stream.exeBranchInfo.pc)] = 1;
            } else {
                find_it->second++;
            }
            ...
            //     }
            //     DPRINTF(DecoupleBP || debugFlagOn, "commit mispredicted
            //     stream %lu\n", it->first);
            // }
        }

        if (/* stream.startPC == ObservingPC &&  */ stream.squashType ==
            SQUASH_CTRL) {
...
FetchStream preStream;

void DecoupledBPUWithFTB::decodeBranch(const DynInstPtr &inst) {
    auto it = fetchStreamQueue.find(inst->fsqId);
    assert(it != fetchStreamQueue.end());
    auto entry = it->second;
    if (inst->isCondCtrl()) {
        it->second.containCond = true;
    }
    if (inst->isIndirectCtrl()) {
        it->second.containIndirect = true;
    }
    if (inst->isDirectCtrl()) {
        it->second.containDirect = true;
    }
    if (inst->isControl()) {
        it->second.containBranch = true;
    }
    if (inst->getPC() == entry.startPC) {
        // DPRINTF(MDEBUG2, "%ld-[%ld, %ld) --> %ld, taken: %i, straight: %d %d,
        // type: %d, containBranch: %d, valid: %d\n",
        //             preStream.startPC, preStream.getBranchInfo().pc,
        //             preStream.getEndPC(), preStream.getTakenTarget(),
        //             preStream.getTaken(), preStream.straightVaild,
        //             preStream.straightTimes, preStream.preBranchType,
        //             preStream.containBranch, preStream.predFTBEntry.valid);
        dbpFtbStats.decodeAllStream++;
        dbpFtbStats.containBranchStream += preStream.containBranch;
    }
    preStream = it->second;
}

void DecoupledBPUWithFTB::commitBranch(const DynInstPtr &inst, bool miss) {
    // do overall statistics
    if (inst->isUncondCtrl()) {
        addCfi(
            branch_prediction::ftb_pred::DecoupledBPUWithFTB::CfiType::UNCOND,
            miss);
    }
    if (inst->isCondCtrl()) {
        addCfi(branch_prediction::ftb_pred::DecoupledBPUWithFTB::CfiType::COND,
               miss);
    }
    if (inst->isReturn()) {
        addCfi(
            branch_prediction::ftb_pred::DecoupledBPUWithFTB::CfiType::RETURN,
            miss);
    } else if (inst->isIndirectCtrl()) {
        addCfi(branch_prediction::ftb_pred::DecoupledBPUWithFTB::CfiType::OTHER,
               miss);
    }
    DPRINTF(DBPFTBStats, "inst=%s\n",
            inst->staticInst->disassemble(inst->pcState().instAddr()));
    DPRINTF(DBPFTBStats,
            "isUncondCtrl=%d, isCondCtrl=%d, isReturn=%d, isIndirectCtrl=%d\n",
            inst->isUncondCtrl(), inst->isCondCtrl(), inst->isReturn(),
            inst->isIndirectCtrl());
    ...
    BranchInfo info(branchAddr, targetAddr, inst->staticInst,
                    fallThruPC - branchAddr);
    bool taken = rv_pc.branching();
    taken |= inst->isUncondCtrl();
    auto find_it =
        topMispredictsByBranch.find(std::make_pair(branchAddr, info.getType()));
    MispredType mtype = FAKE_LAST;
    // branchLastDirection[branchAddr] = taken;
    if (miss) {
        ...
        if (!taken) {
            ...
            if (entry.isHit) {
                for (auto &slot : entry.predFTBEntry.slots) {
                    if (slot.pc == branchAddr &&
                        slot.getType() == info.getType()) {
        ...
        DPRINTF(
            Profiling,
            "branchAddr %#lx is mispredicted, taken %d, type %d, missType %d\n",
            branchAddr, taken, info.getType(), mtype);
        assert(mtype != FAKE_LAST);
    }
    DPRINTF(Profiling,
            "lookup topMispredictsByBranch for branchAddr %#lx, type %d\n",
    ...
    if (find_it == topMispredictsByBranch.end()) {
        ...
        topMispredictsByBranch[std::make_pair(branchAddr, info.getType())] =
            std::make_pair(desc, 1);
    ...
    if (taken) {
        ...
        DPRINTF(Profiling,
                "lookup currentPhaseTakenBranches for taken branchAddr %#lx\n",
                branchAddr);
        ...
        DPRINTF(
            Profiling,
            "lookup currentSubPhaseTakenBranches for taken branchAddr %#lx\n",
            branchAddr);
    ...
    for (int i = 0; i < numBr; i++) {
        if (entry.loopRedirectInfos[i].branch_pc ==
    ...
    if (targetAddr < branchAddr || lp.findLoopBranchInStorage(branchAddr)) {
        ...
        if (enableDB) {
            rec.set_outside_lp(branchAddr, targetAddr, miss,
                               predLoopEntry.specCnt, predLoopEntry.tripCnt,
                               predLoopEntry.conf);
    ...
    for (int i = 0; i < numBr; i++) {
        if (entry.loopRedirectInfos[i].branch_pc ==
            ...
            if (loopEntry.specCnt == loopEntry.tripCnt ||
                (loopEntry.specCnt == loopEntry.tripCnt - 1 &&
                 entry.isDouble)) {
...
void DecoupledBPUWithFTB::notifyInstCommit(const DynInstPtr &inst) {
    auto it = fetchStreamQueue.find(inst->fsqId);
    assert(it != fetchStreamQueue.end());
    ...
    if (numInstCommitted % phaseSizeByInst == 0) {
        ...
        if (phaseIdToDump <= currentPhaseID) {
            ...
            // FIXME: parameterize
            for (int i = 0; i <= 16; i++) {
                currentPhaseFsqEntryNumCommittedInstDist[i] =
                    commitFsqEntryHasInstsVector[i] -
                    lastPhaseFsqEntryNumCommittedInstDist[i];
                lastPhaseFsqEntryNumCommittedInstDist[i] =
                    commitFsqEntryHasInstsVector[i];
                currentPhaseFsqEntryNumFetchedInstDist[i] =
                    commitFsqEntryFetchedInstsVector[i] -
                    lastPhaseFsqEntryNumFetchedInstDist[i];
                lastPhaseFsqEntryNumFetchedInstDist[i] =
                    commitFsqEntryFetchedInstsVector[i];
            }
            fsqEntryNumCommittedInstDistByPhase.push_back(
                currentPhaseFsqEntryNumCommittedInstDist);
            fsqEntryNumFetchedInstDistByPhase.push_back(
                currentPhaseFsqEntryNumFetchedInstDist);
            ...
            for (auto &it : topMispredictsByBranch) {
                ...
                if (last_it != lastPhaseTopMispredictsByBranch.end()) {
                    ...
                    for (int i = 0; i < FAKE_LAST; i++) {
                        missMap[MispredType(i)] -=
                            last_it->second.first.second[MispredType(i)];
                    }
                }
                if (total > 0) {
                    currentPhaseTopMispredictsByBranch[it.first] =
                        std::make_pair(std::make_pair(miss, missMap), total);
                }
            }
            lastPhaseTopMispredictsByBranch = topMispredictsByBranch;
            topMispredictsByBranchByPhase.push_back(
                currentPhaseTopMispredictsByBranch);
            ...
            for (auto &it : totalFTBEntries) {
                ...
                // use new entries, what if entry of the same start addr
                // changes?
                if (visit_cnt > 0) {
                    currentPhaseFTBEntries[it.first] =
                        std::make_pair(entry, visit_cnt);
                }
            }
    ...
    if (numInstCommitted % subPhaseSizeByInst()) {
        ...
        if (subPhaseIdToDump <= currentSubPhaseID) {
            ...
            for (auto &it : topMispredictsByBranch) {
                ...
                auto total = it.second.second;
                auto last_it =
                    lastSubPhaseTopMispredictsByBranch.find(it.first);
                ...
                if (total > 0) {
                    currentSubPhaseTopMispredictsByBranch[it.first] =
                        std::make_pair(std::make_pair(miss, missMap), total);
                }
            }
            lastSubPhaseTopMispredictsByBranch = topMispredictsByBranch;
            topMispredictsByBranchBySubPhase.push_back(
                currentSubPhaseTopMispredictsByBranch);
...
void DecoupledBPUWithFTB::squashStreamAfter(unsigned squash_stream_id) {
    auto erase_it = fetchStreamQueue.upper_bound(squash_stream_id);
    int eraseNum = 0;
    Addr eraseAddr = 0;
    if (erase_it != fetchStreamQueue.end()) {
        eraseAddr = erase_it->second.startPC;
    }
    while (erase_it != fetchStreamQueue.end()) {
        eraseNum++;
        printStream(erase_it->second);
        if (enableLoopPredictor) {
            DPRINTF(LoopPredictorVerbose,
                    "recovering loop entry in stream %lu\n", erase_it->first);
            for (int i = 0; i < numBr; i++) {
                auto &loopInfo = erase_it->second.loopRedirectInfos[i];
                DPRINTF(LoopPredictorVerbose,
                        "loop entry %d: pc %#lx, endLoop %d, specCnt %d, "
                        "tripCnty %d, conf %d\n",
                        i, loopInfo.branch_pc, loopInfo.end_loop,
                        loopInfo.e.specCnt, loopInfo.e.tripCnt,
                        loopInfo.e.conf);
                if (loopInfo.e.valid) {
            ...
            for (auto &info : erase_it->second.unseenLoopRedirectInfos) {
                DPRINTF(LoopPredictorVerbose,
                        "ftb unseen loop entry %d: pc %#lx, endLoop %d, "
                        "specCnt %d, tripCnty %d, conf %d\n",
                        j + numBr, info.branch_pc, info.end_loop,
                        info.e.specCnt, info.e.tripCnt, info.e.conf);
                if (info.e.valid) {
    ...
    DPRINTF(MDEBUG2, "erase stream num: %d, addr: %ld\n", eraseNum, eraseAddr);
}

void DecoupledBPUWithFTB::dumpFsq(const char *when) {
...
void DecoupledBPUWithFTB::tryEnqFetchStream() {
    defer _(nullptr, std::bind([this] { debugFlagOn = false; }));
    if (s0PC == ObservingPC) {
        debugFlagOn = true;
    }
...
void DecoupledBPUWithFTB::setTakenEntryWithStream(
    const FetchStream &stream_entry, FtqEntry &ftq_entry) {
    ...
    ftq_entry.iter = stream_entry.isDouble         ? 2
                     : stream_entry.fromLoopBuffer ? 1
                                                   : 0;
    ftq_entry.isExit = stream_entry.isExit;
    ftq_entry.loopEndPC = stream_entry.getBranchInfo().getEnd();
}

void DecoupledBPUWithFTB::setNTEntryWithStream(FtqEntry &ftq_entry,
                                               Addr end_pc) {
...
void DecoupledBPUWithFTB::tryEnqFetchTarget() {
    ...
    if (enableJumpAheadPredictor) {
        ...
        if (jaHit) {
            ...
            thisFtqEntryShouldEndPC =
                stream_to_enq.startPC + (currentSentBlock + 1) * 0x20;
            currentSentBlock++;
        }
    }
    ...
    ftq_enq_state.pc =
        inLoop  ? loopExit ? loopEndPC : stream_to_enq.getBranchInfo().target
        : taken ? stream_to_enq.getBranchInfo().target
                : thisFtqEntryShouldEndPC;

    // we should not increment streamId to enqueue when ja blocks are not fully
    // consumed
    if (!(enableJumpAheadPredictor && stream_to_enq.jaHit &&
          stream_to_enq.jaHit &&
          stream_to_enq.currentSentBlock <
              stream_to_enq.jaEntry.jumpAheadBlockNum)) {
        ftq_enq_state.streamId++;
    }
    DPRINTF(DecoupleBP,
    ...
    //         stream_to_enq.getEnded() ? "n ended" : " miss",
    //         ftq_enq_state.nextEnqTargetId);
    printFetchTarget(ftq_entry, "Insert to FTQ");
    fetchTargetQueue.dump("After insert new entry");
}

void DecoupledBPUWithFTB::histShiftIn(int shamt, bool taken,
...
void DecoupledBPUWithFTB::makeLoopPredictions(
    FetchStream &entry, bool &endLoop, bool &isDouble, bool &loopConf,
    std::vector<LoopRedirectInfo> &lpRedirectInfos,
    ...
    if (finalPred.valid) {
        ...
        for (auto &slot : finalPred.ftbEntry.slots) {
            if (slot.isCond && (finalPred.getTakenBranchIdx() >= i ||
                                finalPred.getTakenBranchIdx() == -1)) {
                assert(finalPred.condTakens.size() > i);
                bool this_cond_pred_taken = finalPred.condTakens[i];
                std::tie(endLoop, lpRedirectInfos[i], isDouble, loopConf) =
                    lp.shouldEndLoop(this_cond_pred_taken, slot.pc, false);
                // for bpu predicted taken branch we need to check
                // whether it is an undetected loop exit
                if (lpRedirectInfos[i].e.valid) {
                    if (loopConf) {
                        // we should only modify the direction of the loop
                        // branch, because a latter branch (outside loop branch)
                        // may have other situation
                        finalPred.condTakens[i] = !endLoop;
                        if (endLoop) {
                            DPRINTF(DecoupleBP || debugFlagOn,
                                    "Loop predictor says end loop at %#lx\n",
                                    slot.pc);
                            ...
                            if (!this_cond_pred_taken) {
                                ...
                                fixNotExits[i] = true;
                                DPRINTF(DecoupleBP || debugFlagOn,
                                        "Loop predictor says do not end loop "
                                        "at %#lx\n",
                                        slot.pc);
                            }
                        }
    ...
    for (Addr pc = entry.startPC; pc < end; pc += 2) {
        ...
        std::tie(endLoop, unseenLpInfo, isDouble, loopConf) =
            lp.shouldEndLoop(false, pc, false);
...
void DecoupledBPUWithFTB::makeNewPrediction(bool create_new_stream) {
    ...
    if (finalPred.controlAddr() == ObservingPC ||
        finalPred.controlAddr() == ObservingPC2) {
        debugFlagOn = true;
    }
    DPRINTF(DecoupleBP || debugFlagOn,
            "Make pred with %s, pred valid: %i, taken: %i\n",
            create_new_stream ? "new stream" : "last missing stream",
            finalPred.valid, finalPred.isTaken());
    // preBranchType需要update
    bool normalIsTaken = finalPred.isTaken();
    bool notGenerateBubble = finalPred.setBranchType(preBranchType);
    if (!enabletbit && !notGenerateBubble && enablenbt) {
        numOverrideBubbles += 3;
    }
    entry.preBranchType = finalPred.preBranchType;

    // if loop buffer is not activated, use normal prediction from branch
    // predictors
    ...
    if (!enableLoopBuffer || (enableLoopBuffer && !lb.isActive())) {
        ...
        if (predReasonable) {
            if (enableLoopPredictor) {
                makeLoopPredictions(entry, endLoop, isDouble, loopConf,
                                    lpRedirectInfos, fixNotExits,
                                    unseenLpRedirectInfos, taken);
            }
            ...
            if (taken) {
                ...
                entry.predBranchInfo.target =
                    nextPC; // use the final target which may be not from ftb
            }
            s0PC = nextPC;
        } else {
            DPRINTF(DecoupleBP || debugFlagOn,
                    "Prediction is not reasonable, printing ftb entry\n");
        ...
        if (enableJumpAheadPredictor) {
            ...
            JAEntry jaEntry;
            Addr jaTarget;
            std::tie(jaHit, jaConf, jaEntry, jaTarget) =
                jap.lookup(entry.startPC);
            ...
            if (!finalPred.valid) {
                if (jaHit && jaConf) {
                    ...
                    s0PC = jaTarget;
                    dbpFtbStats.predJATotalSkippedBlocks +=
                        jaEntry.jumpAheadBlockNum - 1;
                    dbpFtbStats.predJASkippedBlockNum.sample(
                        jaEntry.jumpAheadBlockNum - 1, 1);
                }
            }
        }
        ...
        entry.highConf = finalPred.isHigh();
        ...
        historyManager.addSpeculativeHist(entry.startPC, shamt, taken,
                                          entry.predBranchInfo, fsqId);
        ...
        std::tie(endLoop, lpRedirectInfos[0], isDouble, loopConf) =
            lp.shouldEndLoop(true, lb.getActiveLoopBranch(),
                             lb.activeLoopMayBeDouble());
    ...
    // tbit
    if(enabletbit) {
        bool prevent = tbit->prevent(entry.startPC);
        if(prevent){
            // entry.predTaken = true;
            // s0PC = tbit->getTarget();

            dbpFtbStats.condSaveTime++;
            dbpFtbStats.indirectSaveTime++;
        }
    }

    if (finalPred.valid) {
        dbpFtbStats.finalPredHit++;
    } else {
        dbpFtbStats.finalPredMiss++;
    }
    entry.loopRedirectInfos = lpRedirectInfos;
    entry.fixNotExits = fixNotExits;
    entry.unseenLoopRedirectInfos = unseenLpRedirectInfos;
    if (notGenerateBubble && !enabletbit &&
        (entry.preBranchType == INDIRECT || entry.preBranchType == DIRECT)) {
        dbpFtbStats.condSaveTime++;
    }
    if (notGenerateBubble && !enabletbit &&
        (entry.preBranchType == CONDITION || entry.preBranchType == DIRECT)) {
        dbpFtbStats.indirectSaveTime++;
    }
    if (preEntry.isHit) {
        if (finalPred.generateBranchType() == INDIRECT) {
            dbpFtbStats.indirectStream++;
        } else if (finalPred.generateBranchType() == CONDITION) {
            dbpFtbStats.condStream++;
        } else if (finalPred.generateBranchType() == DIRECT) {
            dbpFtbStats.directStream++;
        } else {
            dbpFtbStats.allStream++;
        }
    } else {
        dbpFtbStats.noBranchStream++;
    }
    switch (noBranchTimes) {
    case 1:
        dbpFtbStats.noBranch1++;
        break;
    case 2:
        dbpFtbStats.noBranch2++;
        break;
    case 3:
        dbpFtbStats.noBranch3++;
        break;
    case 4:
        dbpFtbStats.noBranch4++;
        break;
    case 5:
        dbpFtbStats.noBranch5++;
        break;
    default: {
        if (noBranchTimes != 0) {
            dbpFtbStats.noBranchAbove++;
        }
    }
    }
    if (entry.isHit) {
    ...
    bool prestraightValid = straightValid;
    if (!notGenerateBubble) {
        bool containBranch = false;
        bool condValid = false;
        bool indirectValid = false;
        bool isDirect = false;
        BranchType currentType = finalPred.ftbEntry.getEntryTypePredict(
            containBranch, condValid, indirectValid, isDirect);
        if (selectType == 0) {
            uftb->setBranchType(preStartPC, entry.startPC, currentType);
            ftb->setBranchType(preStartPC, entry.startPC, currentType);
        } else if (selectType == 2) {
            directStream->updateType(straightPC, currentType);
        } else {
            straightValid = false;
            directStream->clear(straightPC);
        }
    }
    bool straightLast = straightValid && straightTimes == 1;
    BranchType preStraightType = straightType;
    if (straightValid) {
        straightTimes--;
        if (straightTimes == 0) {
            straightValid = finalPred.directValid &&
                            directStream->compare(finalPred.directAddr,
                                                  finalPred.getBranchAddr());
            straightTimes = finalPred.directTimes;
            straightType = (BranchType)finalPred.directType;
            straightPC = entry.startPC;
        }
    } else {
        straightValid = finalPred.directValid &&
                        directStream->compare(finalPred.directAddr,
    ...
    if (enablenst) {
        if (straightLast && !finalPred.valid) {
            preBranchType = preStraightType;
            selectType = 2;
        } else if (straightValid) {
            preBranchType = DIRECT;
            selectType = 1;
        } else {
            preBranchType = finalPred.generateBranchType();
            selectType = 0;
        }
    } else {
        preBranchType = finalPred.generateBranchType();
    ...
    entry.straightVaild = straightValid;
    entry.straightTimes = straightTimes;
    entry.directBranchAddr = finalPred.getBranchAddr();

    if (!notGenerateBubble && fetchTargetQueue.size() < 2) {
        DPRINTF(MDEBUG2, "stream squash\n");
    }

    DPRINTF(
        MDEBUG,
        "%ld-[%ld, %ld) --> %ld, taken: %i, straight: %d %d, branchType: %d\n",
        entry.startPC, entry.getBranchInfo().pc, entry.getEndPC(),
        entry.getTakenTarget(), entry.getTaken(), straightValid, straightTimes,
        entry.preBranchType);
    DPRINTF(MDEBUG2,
            "%ld-[%ld, %ld) --> %ld, taken: %i, bubble: %d, straight: %d %d, "
            "type: %d %d\n",
            entry.startPC, entry.getBranchInfo().pc, entry.getEndPC(),
            entry.getTakenTarget(), entry.getTaken(), notGenerateBubble,
            straightValid, straightTimes, selectType, entry.preBranchType);
    preStartPC = entry.startPC;
    ...
    Addr jumpAddr = 0;
    if (entry.predTaken) {
        jumpAddr = entry.predBranchInfo.pc;
    }
    // auto entrySlotInfo = slotTargetMap.find(entry.startPC);
    // if (entrySlotInfo != slotTargetMap.end()){
    //     auto slotInfo = entrySlotInfo->second.find(jumpAddr);
    //     if (slotInfo != entrySlotInfo->second.end()){
    //         slotInfo->second++;
    //     }
    //     else{
    //         entrySlotInfo->second[jumpAddr] = 1;
    //     }
    // }
    else {
        std::map<Addr, int> entrySlots;
        entrySlots[jumpAddr] = 1;
        // slotTargetMap[entry.startPC] = entrySlots;
    }
...
void DecoupledBPUWithFTB::checkHistory(
    const boost::dynamic_bitset<>
        &history) { /*
                       unsigned ideal_size = 0;
                       boost::dynamic_bitset<> ideal_hash_hist(historyBits, 0);
                       int ideal_sp = ras->getNonSpecSp();
                       std::vector<RAS::RASEntry>
                       ideal_stack(ras->getNonSpecStack()); for (const auto
                       entry: historyManager.getSpeculativeHist()) { if
                       (entry.shamt == 0 && !entry.is_call && !entry.is_return)
                       { continue;
                           }
                           if (entry.shamt != 0) {
                               ideal_size += entry.shamt;
                               DPRINTF(DecoupleBPVerbose, "pc: %#lx, shamt: %d,
                       cond_taken: %d\n", entry.pc, entry.shamt,
                       entry.cond_taken); ideal_hash_hist <<= entry.shamt;
                               ideal_hash_hist[0] = entry.cond_taken;
                           }
                           if (entry.is_call || entry.is_return) {
                               DPRINTF(DecoupleBPVerbose, "pc: %#lx, is_call:
                       %d, is_return: %d, retAddr: %#lx\n", entry.pc,
                       entry.is_call, entry.is_return, entry.retAddr); if
                       (entry.is_call) { ras->push(entry.retAddr, ideal_stack,
                       ideal_sp);
                               }
                               if (entry.is_return) {
                       ...
                       unsigned comparable_size = std::min(ideal_size,
                       historyBits); boost::dynamic_bitset<>
                       sized_real_hist(history);
                       ...
                       DPRINTF(DecoupleBP,
                               "Ideal size:\t%u, real history size:\t%u,
                       comparable size:\t%u\n", ideal_size, historyBits,
                       comparable_size); DPRINTF(DecoupleBP, "Ideal
                       history:\t%s\nreal history:\t%s\n", buf1.c_str(),
                       buf2.c_str()); int sp = ras->getSp(); DPRINTF(DecoupleBP,
                       "ideal sp:\t%d, real sp:\t%d\n", ideal_sp, sp);
                       assert(ideal_hash_hist == sized_real_hist);
                       // assert(ideal_sp == sp);
                       */
}

void DecoupledBPUWithFTB::resetPC(Addr new_pc) {
    s0PC = new_pc;

...
numBr(p.numBr)
{
    DPRINTF(FTBITTAGE || debugFlag, "FTBITTAGE constructor numBr=%d\n", numBr);
        bankStats = new ITTageBankStats(this,
            (std::string("ittage")).c_str(),
            numPredictors);
    ...
    if (should_update) {
        ...
        if (mainFound) { // updateProvided
            ...
            if (way.counter == 0) {
                ...
                bankStats->updateTimes[pred.main_table]++;
            }
            ...
            if (pred.useAlt && mispred) {
                ...
                if (alt_way.counter == 0) {
                    ...
                    bankStats->updateTimes[pred.alt_table]++;
                }
            }
        }
...
FTBITTAGE::ITTageBankStats::ITTageBankStats(statistics::Group* parent, const char *name, int numPredictors):
    statistics::Group(parent, name),
    ADD_STAT(updateTimes, statistics::units::Count::get(), "all ittage table update times")
{
    updateTimes.init(numPredictors);
}

...
public:
    struct HistoryEntry
    {
        HistoryEntry(Addr _pc, int _shamt, bool _cond_taken, bool _is_call,
                     bool _is_return, Addr _retAddr, uint64_t stream_id)
            : pc(_pc), shamt(_shamt), cond_taken(_cond_taken),
              is_call(_is_call), is_return(_is_return), retAddr(_retAddr),
              streamId(stream_id) {}
        Addr pc;
        Addr shamt;
        bool cond_taken;
...
public:
    void addSpeculativeHist(const Addr addr, const int shamt, bool cond_taken,
                            BranchInfo &bi, const uint64_t stream_id) {
    ...
    void printEntry(const char *when, const HistoryEntry &entry) {
        DPRINTF(DecoupleBPVerbose,
                "%s stream: %lu, pc %#lx, shamt %d, cond_taken %d, is_call %d, "
                "is_ret %d, retAddr %#lx\n",
...
private:
    ...
    ftb_pred::RAS *ras{};
    ftb_pred::uRAS *uras{};
    ftb_pred::DirectStream *directStream;
    ...
    void printStream(const FetchStream &e) {
        ...
        // TODO:fix this
        DPRINTFR(DecoupleBP, "%#lx-[%#lx, %#lx) --> %#lx, taken: %i\n",
    ...
    void printFTBEntry(const FTBEntry &entry) {
        ...
        for (auto &slot : entry.slots) {
            DPRINTF(FTB,
                    "    pc:%#lx, size:%d, target:%#lx, cond:%d, indirect:%d, "
                    "call:%d, return:%d\n",
    ...
    struct DBPFTBStats : public statistics::Group
    {
        ...
        statistics::Scalar finalPredHit;
        statistics::Scalar finalPredMiss;
        ...
        statistics::Scalar predTimes;
        statistics::Scalar condSaveTime;
        statistics::Scalar indirectSaveTime;
        statistics::Scalar noBranchStream;
        statistics::Scalar indirectStream;
        statistics::Scalar directStream;
        statistics::Scalar condStream;
        statistics::Scalar allStream;
        statistics::Scalar noBranch1;
        statistics::Scalar noBranch2;
        statistics::Scalar noBranch3;
        statistics::Scalar noBranch4;
        statistics::Scalar noBranch5;
        statistics::Scalar noBranchAbove;
        statistics::Scalar decodeAllStream;
        statistics::Scalar containBranchStream;
...
public:
    ...
    FtqEntry &getSupplyingFetchTarget() { return fetchTargetQueue.getTarget(); }
    ...
    enum MispredType
    {
        DIR_WRONG, TARGET_WRONG, NO_PRED, FAKE_LAST };
    ...
    std::map<Addr, std::map<Addr, int>> slotTargetMap;
    ...
    void setTakenEntryWithStream(const FetchStream &stream_entry,
                                 FtqEntry &ftq_entry);
    ...
    void addFtqNotValid() { dbpFtbStats.ftqNotValid++; }

    void commitBranch(const DynInstPtr &inst, bool miss);
    void decodeBranch(const DynInstPtr &inst);
